		...
		<view class="container" v-show="!isLoading">
			<scroll-view scroll-y="true">
				...
				<view class="list-music">
					...
					<view class="list-music-item" v-for="(item,index) in playlist.tracks" :key='index' @tap="headToDetail(item.id)">
						<view class="list-music-top">{{index + 1}}</view>
						<view class="list-music-song">
							<view>{{ item.name }}</view>
							<view>
								<image  v-if="privileges[index].flag > 60 && privileges[index].flag < 70" src="../../static/shiting.png" mode=""></image>
